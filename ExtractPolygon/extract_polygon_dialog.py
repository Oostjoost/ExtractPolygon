...
class ExtractPolygonDialog(QtGui.QDialog, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        self.get_shapedir.clicked.connect(self.get_output_directory)
    ...
    def saga_map_calculator(self, tempLayer):
        ...
        output_alg = processing.runandload('saga:rastercalculator', tempLayer , '', rasterFilterSaga, False, 7, None)
        
    def gdal_polygonize(self, layer, f):
        output_file = f + '/temp_pol.shp'
        result = processing.runandload('gdalogr:polygonize', layer, "DN", "test1", "test2", self.outputFile)
    ...
    def get_input_file(self):
        ...
        if self.inputFile != None:
            self.check_input[0] = 1
            self.input_geotiff.setText(str(self.inputFile))
        if sum(self.check_input) == 2:
            self.runButton.setEnabled(True)
        
    def get_output_directory(self):
        qfd = QFileDialog()
        title = 'Save output file as .shp file'
        path = str(os.path)
        self.outputFile = QFileDialog.getSaveFileName(qfd, title, path, '*.shp')
        if self.outputFile != None:
            self.check_input[1] = 1
            self.output_dir.setText(str(self.outputFile))
        if sum(self.check_input) == 2:
            self.runButton.setEnabled(True)        
    ...
    def run(self):
        ...
        vect_layer = self.get_layer_by_name(layer_name)
        #output = QgsVectorFileWriter.writeAsVectorFormat(vect_layer, self.outputFile,"utf-8",None,"ESRI Shapefile")
        #vect_layer.setLayerName('liander_t0')
